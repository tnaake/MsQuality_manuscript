...
## batch 1
## read the file with protein intensities
.path <- "/scratch/naake/data_QC/1_of_5"
fls <- dir(.path, full.names = TRUE)
...
## batch 2
## read the file with protein intensities
.path <- "/scratch/naake/data_QC/2_of_5"
fls <- dir(.path, full.names = TRUE)
...
## batch 3
## read the file with protein intensities
.path <- "/scratch/naake/data_QC/3_of_5"
fls <- dir(.path, full.names = TRUE)
...
## batch 4
## read the file with protein intensities
.path <- "/sratch/naake/data_QC/4_of_5"
fls <- dir(.path, full.names = TRUE)
...
## batch 5
## read the file with protein intensities
.path <- "/scratch/naake/data_QC/5_of_5"
fls <- dir(.path, full.names = TRUE)
...
```{r echo = FALSE, eval = FALSE}
## truncate the dataOrigin
dataOrigin(sps) <- gsub(dataOrigin(sps), pattern = "\\", 
    replacement = "/", fixed = TRUE) |>
    stringr::str_remove(pattern = "C:/Users/naake/Documents/Publications/") |>
    stringr::str_remove(pattern = "Bioinformatics_MsQuality/data/peak/New_mzMLFinal/") |>
    stringr::str_remove(pattern = ".mzML") |>
...
saveRDS(.metrics_sps, file = "data/metrics_sps.RDS")
```
