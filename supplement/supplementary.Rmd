...
metrics of mass spectrometry data". It demonstrates functionality of the 
`MsQuality` package on two example analysis 
...
```{r set_directory_for, cache = FALSE, warning=FALSE}
...
```{r biocFileCache_cherkaoui2022, message = FALSE, cache = FALSE, warning=FALSE}
...
```{r metrics-cherkaoui2022, fig.cap = "Quality metrics for data set of @Cherkaoui2022 stratified by information if the measurement was analyzed or excluded. (a) Area under the TIC (`areaUnderTic`). (b) Minimum values of the m/z acquisition range (`mzAcquisitionRange.min`). (c) maximum values of the m/z acquisition range `mzAcquisitionRange.max`). A.U.: arbitrary units.", echo = FALSE, cache = FALSE}
...
```{r helper_function_plot_beeswarm, eval = TRUE, echo = FALSE, warning = FALSE}
library(patchwork)
