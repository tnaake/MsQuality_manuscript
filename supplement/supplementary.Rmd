...
This is the Supplementary Information for the publication "FIXME TITLE". It demonstrates  
...
mzQC framework @CitationFIXME is extensive, also including metrics that depend on  
...
We note that these quality metrics are indicative, but by themselves might not be sufficient
for data quality control decision-making, such as removing low-quality measurements, which might require additional
...
be compared with regard to the same sample type, experimental setup, 
instrumentation, etc. FIXME: seems like a lot of shallow philosophy and obviousisms here, is this needed for a supplement that should just get to the point and show the demo?
...
FIXME: needless and redundant-with-the-code blabla?
...
were executed within a high-performance computing environment<!-- FIXME whatever that means... if you need to say this at all, please be concrete, i.e. number of cores, TB RAM, etc. A HPC from 2005 is probably weaker than my current telephone... -->, where the modified<!-- FIXME how modified? --> .mzML files were stored in the directory `Cherkaoui2022`.
...
<!-- FIXME Please here and elsewhere use less cumbersome and bureaucratic language, e.g., replace the following with
"The metrics are calculated using the function" 
You may use ChatGPT for help. -->
...
can be passed to `calculateMetricsFromSpectra` to further control the 
calculation, such as the parameter `msLevel` if multiple mass spectra levels are present 
in the `Spectra` object. In the current context, where only MS1 level spectra 
are stored in the `Spectra` object, specifying `msLevel` is not necessary.

<!-- FIXME: Why do you use variable names that begin with "." ???

Also, why so many chunks with eval=FALSE? This destroys the reproducibility idea. I'd rather have a vignette
that only runs with a big computer and needs some time, than one that does not run at all, or needs lots of
...
<!-- FIXME In the following paragraph, the use of the passive voice is confusing, since it is not clear who did what.
Better switch to active voice. -->
...
to extract information on the measurements that were included in the subsequent 
...
```{r areaUnderTicRtQuantiles-amidan2014, fig.cap = "Quality metrics by `MsQuality`: Area under TIC RT quantiles (`areaUnderTicRtQuantiles`). The `MsQuality` metrics are calculated from MS1 and MS2 spectra. One data point is obtained per MS1 and MS2 measurement run and the data points are displayed as 2D densities stratified for high-quality and low-quality measurements as classified in @Amidan2014. (a) 25% quantile for MS1 spectra. (b) 25% quantile for MS2 spectra. (c) 50% quantile for MS1 spectra. (d) 50% quantile for MS2 spectra.E: 75% quantile for MS1 spectra. (f) 75% quantile for MS2 spectra. (g) 100% quantile for MS1 spectra. (h) 100% quantile for MS2 spectra.", warning = FALSE, echo = FALSE, fig.height = 7}
xlim <- c(2.5, 7.5)
